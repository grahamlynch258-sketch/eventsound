...
// County regions as simplified paths
const COUNTY_PATHS = [
  // Ulster (Northern Ireland + Donegal, Cavan, Monaghan)
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="max-w-3xl mx-auto text-center mb-10">
          <h2 className="text-3xl font-bold text-foreground mb-4 transition-transform duration-300 hover:scale-[1.05] cursor-default">
            Find AV Services Near You
          </h2>
          <p className="text-muted-foreground leading-relaxed">
            EventSound provides LED wall hire and conference AV services across Ireland. Select a city below to learn more about our services in your area.
          </p>
        </div>
        ...
        <div className="relative max-w-md mx-auto">
          <svg
            ...
            role="img"
            aria-label="Interactive map of Ireland showing EventSound service locations"
          >
            {/* County fills */}
            ...
            {CITIES.map((city) => (
              <g key={city.name}>
                ...
                <circle
                  ...
                  onMouseEnter={() => {
                    if (!activeCity) {
                      setTooltip({ text: `${city.name} — LED Walls & Conference AV`, x: city.x, y: city.y - 20 });
                    }
                  }}
                  onMouseLeave={() => setTooltip(null)}
                />
            ...
            {activeCity && (() => {
              ...
              return (
                <motion.div
                  ...
                  className="absolute z-30 bg-card border border-accent/30 rounded-xl p-4 shadow-xl w-56"
                  style={{
                    left: `${(city.x / 410) * 100}%`,
                    top: `${(city.y / 480) * 100}%`,
                ...
                >
                  <p className="text-sm font-semibold text-accent mb-3">{city.name}</p>
                  <div className="space-y-2">
                    <Link
                      to={city.ledWallsPath}
                      className="block w-full text-center text-sm px-3 py-2 rounded-lg bg-accent/10 border border-accent/20 text-foreground hover:bg-accent/20 transition-colors"
                    >
                      LED Wall Hire
                    </Link>
                    <Link
                      to={city.conferenceAvPath}
                      className="block w-full text-center text-sm px-3 py-2 rounded-lg bg-accent/10 border border-accent/20 text-foreground hover:bg-accent/20 transition-colors"
                    >
                      Conference AV
                    </Link>
