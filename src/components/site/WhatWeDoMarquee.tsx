...
export function WhatWeDoMarquee({ items, intervalSec = 2 }: WhatWeDoMarqueeProps) {
  ...
  const handleTransitionEnd = useCallback(() => {
        ...
        setSlideOffset(0);
        setAnimating(false);
        // After animation ends, re-check if mouse is still hovering
                                              // If it is, keep paused; if not, unpause
  ...
  return (
        <div
                ...
                onMouseLeave={() => {
                          ...
                          // Only unpause if not currently animating
                          // If animating, handleTransitionEnd will check hoveredRef and unpause
...
function ServiceCard({ item }: { item: MarqueeItem }) {
    ...
    const card = (
          <div
            ...
            {Icon && (
                ...
                <h3 className="font-serif text-xl font-semibold mb-3 text-foreground">{item.label}</h3>h3>
            {item.description && <p className="text-sm text-muted-foreground leading-relaxed flex-1">{item.description}</p>p>}
    ...
    return item.href ? (
          ...
          <div className="h-full">{card}</div>div>
        );
}</div>
